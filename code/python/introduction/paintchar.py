...
# ・ パレット領域を選択して色を指定するように変更
# ・ 描画時に右クリックすると、クリックされた位置の色の領域を指定色で塗りつぶす(fill機能)
# ・ oキーでセーブした画像を読み込む機能(open機能）
# ・ sキーでダイアログからファイル名を選択して書き込む機能(save機能)
# ・ uキーで各処理ごとにundoする機能(undo機能)
# ・ rキーでundoした処理をredoする機能(redo機能)
...
# undoに格納するコマンドバッファクラス	
class UndoBuf:
	def __init__(self):
		self.buf = []
	# コマンドの追加
	def append(self, col, row, old_c, new_c):
		self.buf.append((col, row, old_c, new_c))
	# undoバッファの結合
	def extend(self, buf):
		self.buf.extend(buf.order())
	# 正順
	def order(self):
		return self.buf
	# 逆順
	def reverse(self):
		return reversed(self.buf)
...
class Undo:
	...
	def append(self, buf):
		if len(self.buf) > self.idx:
			del self.buf[self.idx:]
		self.buf.append(buf)
		self.idx = len(self.buf)
	# undo処理
	def undo(self):
		if self.idx > 0:
			buf = self.buf[self.idx-1]
			for col, row, old_c, new_c in buf.reverse():
				mtrx[row][col] = old_c
			self.idx -= 1
	# redo処理
	def redo(self):
		if self.idx < len(self.buf):
			...
			for col, row, old_c, new_c in buf.order():
				mtrx[row][col] = new_c
			self.idx += 1
...
def fill(col, row, old_color, new_color):
	buf = UndoBuf()	# 戻り値のundo用バッファ
	if mtrx[row][col] != old_color:	# 座標が塗りつぶし対象の色でなければ終了
		return buf
	buf.append(col, row, old_color, new_color)	# 元の座標値をundo用配列に追加
	...
	if cut:
		...
		for r in range(row, row+height):
			for c in range(col, col+width):
				buf.append(c, r, mtrx[r][c], WHITE)
				mtrx[r][c] = WHITE
		undo.append(buf)	# cutしても戻せるようにundoバッファに格納
					...
					if cp_mode != CP_MODE_NONE:
							...
							for r in range(height):
								for c in range(width):
									if r+row < matrix_size and c+col < matrix_size:
										buf.append(c+col, r+row, mtrx[r+row][c+col], cp_buf[r][c])
						...
						if pos[1] < block_size * matrix_size: 
							...
							if mtrx[row][col] != color:	# 同じところに二度書きするとundoの動作が不自然になるので、禁止する
								buf = UndoBuf()
								buf.append(col, row, mtrx[row][col], color)
								undo.append(buf)
								mtrx[row][col] = color
					elif pygame.mouse.get_pressed()[2]:	# 右クリックなら塗りつぶし(fill)
						col = math.floor(pos[0] / block_size)
						row = math.floor(pos[1] / block_size)
						if mtrx[row][col] != color:
							undo.append(fill(col, row, mtrx[row][col], color))
