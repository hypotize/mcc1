...
**D$4**のように、**片方だけに$記号が付く**（相対参照と絶対参照が混在）パターンを**複合参照**と呼びます。
...
固定しなければいけないのは`4`の部分です。

#### どうすうればよいのか

それでは、**行を固定**します。

セル**B2**の`=$A2*B1`の`B1`の前または後ろにカーソルを置いて、**F4キー**を**２回**押下します。（もしくは`4`の前に`$`を直接入力する）
...
セル**F6**の計算式を見ても`=$A6*$F1`と正しいセルを参照しています。
